...
#         rclpy.spin_once(node, timeout_sec=0.1)  # Tim
...
#     def mocap_callback(self, msg):
#         self.get_logger().info(f"Received mocap pose data: {msg.transform.translation.x}, {msg.transform.translation.y}")
#         if time.time() - self.start_time < self.duration:
#             self.mocap_data.append({            
#                 'timestamp': msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9,
#                 'translation_x': msg.transform.translation.x,
...
#     def transform_callback(self, msg):
#         self.get_logger().info(f"Received transform data: {msg.transform.translation.x}, {msg.transform.translation.y}")
#         if time.time() - self.start_time < self.duration:
#             self.transform_data.append({
#                 'timestamp': msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9,
#                 'translation_x': msg.transform.translation.x,
...
#     # Continuously collect data for the specified duration
#     while time.time() - node.start_time < duration:
#         rclpy.spin_once(node, timeout_sec=0.1)  # Timeout of 0.1s to periodically check for messages

#     # Convert data to Pandas DataFrames
#     mocap_df = pd.DataFrame(node.mocap_data)
#     transform_df = pd.DataFrame(node.transform_data)
#     mocap_df.to_csv("mocap_csv.csv")
...
#     # Matching the initial origin of orbslam zeros
#     zero_condition=((transform_df['translation_x']==0).values & 
#                     (transform_df['translation_y']==0).values & 
#                     (transform_df['translation_z']==0).values)

#     ff_index=np.argmax(~zero_condition) # Index where orbslam has started giving some data
#     timestamps=transform_df["timestamp"].values
#     timestamp_i=timestamps[0]
#     timestamp_f=timestamps[-1]
#     # timestamp_f=transform_df["timestamp"][-1]
